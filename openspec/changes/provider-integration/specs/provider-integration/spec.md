...
`SwitchSMSProviderToIdentifier` SHALL perform a priority swap: if B.priority > A.priority → swap values; if B.priority == A.priority → A.priority += 10. Both providers get a version bump and history row with NOTIFY_USER_ID as created_by. No-op if target is already current or is inactive.
